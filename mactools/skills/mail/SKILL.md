---
name: mail
description: "读取、搜索、整理 macOS 邮件。支持查看收件箱、读取邮件内容、标记、移动、删除（移废纸篓）。当用户需要整理邮件、查找邮件、清理收件箱时使用。Use when the user needs to organize emails, search for messages, or clean up inbox. Keywords: 邮件, Mail, 收件箱, 整理邮件, 清理邮件."
context: fork
model: haiku
...
| 分类 | 处理方式 |
|------|---------|
| 有用（含待办事项） | 提取 TODO，提示用户运行 `/omnifocus` 创建任务 |
| 参考资料（需留存） | 总结要点，提示用户运行 `/notes` 创建笔记 |
| 垃圾/过期/无用 | 移到废纸篓 |

### Step 3: 创建整理摘要

提示用户运行 `/notes` 创建一条笔记，记录本次整理结果：
- 处理了多少封邮件
- 提取了哪些待办事项
- 保存了哪些参考资料
- 清理了多少封垃圾邮件
