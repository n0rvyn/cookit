...
1. 读取 `references/apple-ui-checklist.md` 获取代码合规检查项
2. 读取 `references/ui-design-principles.md` §13 检查清单 和 §16 页面构成模板
3. 如果目标是 macOS 项目（代码含 `WindowGroup`/`MenuBarExtra`/`NSViewRepresentable`/`#if os(macOS)`），额外读取 `apple-ui-checklist.md` section 12（macOS UI 检查）
...
#### B7. macOS 专属检查（macOS 项目时执行）

> 仅当 Step 2 判定为 macOS 项目时执行。参考 `apple-ui-checklist.md` section 12。

**Window 行为**：
- [ ] 窗口是否可自由调整大小？有无 `.frame(minWidth:minHeight:)`？
- [ ] `WindowGroup` / `Window` / `Settings` 场景选择是否正确？

**Toolbar & Menu**：
- [ ] 主要操作是否同时在 toolbar 和菜单中可达？
- [ ] 右键菜单（contextMenu）是否提供了桌面用户期望的操作？

**键盘导航**：
- [ ] 主要操作是否有 `.keyboardShortcut()` 绑定？
- [ ] 标准快捷键是否遵循惯例？（Cmd+N/W/,）

**Sidebar 导航**：
- [ ] 是否使用 `NavigationSplitView` 做一级导航（而非 TabBar）？
- [ ] Sidebar 选中状态是否有清晰的视觉反馈？
