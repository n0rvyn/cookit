...
1. 读取 `references/ui-design-principles.md` 获取设计原则和精确数值标准
2. 读取 `references/apple-ui-checklist.md` 的「设计质量」章节获取代码级检查项
3. 如果目标是 macOS 项目（代码含 `WindowGroup`/`MenuBarExtra`/`NSViewRepresentable`/`#if os(macOS)`），额外读取 `ui-design-principles.md` section 11b（macOS 平台特定：Window 尺寸、Sidebar、Toolbar）
...
**代码检查**：搜索硬编码 `.padding(N)` 或 `.spacing(N)`，N 不是 4pt 倍数即标记。搜索 `Color(hex:` / `.font(.system(size:` 等非 Token 用法。

> 💡 完成设计审查后，如需完整 Token 合规扫描，单独运行 `/validate-design-tokens`。
...
> 原则参考：`ui-design-principles.md` §4.1 — 弱化不重要的信息，比放大重要信息更有效。
...
> 原则参考：`ui-design-principles.md` §2.1 — 所有间距值基于 4pt 倍数。
...
### A-mac. macOS 设计检查（macOS 项目时执行）

> 仅当 Step 2 判定为 macOS 项目时执行。参考 `ui-design-principles.md` §11.7-11.9。

#### A-mac1. Window 尺寸

- [ ] 主窗口是否设置了合理的 `.defaultSize()` 和 `.frame(minWidth:minHeight:)`？
- [ ] 最小尺寸是否不小于 600x400pt（主窗口）/ 480x320pt（设置窗口）？

#### A-mac2. Sidebar 布局

- [ ] Sidebar 是否使用 `.listStyle(.sidebar)` 获得系统外观？
- [ ] Sidebar 默认宽度是否在 200-250pt 范围？
- [ ] 选中状态是否使用系统高亮色？

#### A-mac3. Toolbar 与菜单一致性

- [ ] Toolbar 中的操作是否同时在 menu commands 中可达？
- [ ] Toolbar 图标是否使用 SF Symbols 且与系统风格一致？
...
- 选项从 Part A 的 🔴 和 🟡 发现中生成
- 用户确认后，仅修复用户选中的项目
- 用户未确认 = 不动代码

## Completion Criteria

- 报告包含 Part A（设计规则，代码验证）和 Part B（视觉打磨，设备验证清单）
- Part A 每个发现附 file:line 引用
- Part B 检查项根据代码分析定制（非通用模板）
- 修复动作经过用户 AskUserQuestion 确认
